...
# MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&aip=1&t=event&ec=dbdemos&ea=VIEW&dp=%2F_dbdemos%2Fdata-science%2Fllm-rag-chatbot%2F_resources%2F00-init&cid=local&uid=local">
...
def wait_for_index_to_be_ready(index_name):
  for i in range(180):
    ...
    if "provisioning" in idx['index_status']['index_state'] :
      if i % 20 == 0: print(f"Waiting for index to build, this can take a few min... {idx['index_status']['message']} - {idx['index_status']['index_state']}")
      time.sleep(10)
    elif "offline" in idx['index_status']['index_state']:
