...
# MAGIC To run this demo, just select the cluster `dbdemos-llm-rag-chatbot-andrew_kraemer` from the dropdown menu ([open cluster configuration](https://e2-demo-field-eng.cloud.databricks.com/#setting/clusters/1027-161315-uhcw9qqo/configuration)). <br />
...
os.environ["DATABRICKS_TOKEN"] = dbutils.secrets.get(
    "dbdemos", "ai_gateway_service_principal"
)
...
class ChatbotRAG(mlflow.pyfunc.PythonModel):
    def load_context(self, context):
        ...
        self.vsc = VectorSearchClientV2(
            token=os.environ["DATABRICKS_TOKEN"], workspace_url=workspace_url
        )
    ...
    def predict(self, context, model_input):
        ...
        for question in model_input:
            ...
            user_prompt = f"Answer the following analyst question. If you don't know or the question isn't relevant or professional, say so. Only give a detailed answer. Don't add note or comment.\n\n  Question: {question}"
            ...
            # if doc is not None: # TODO update with link to transcript location
...
results = proxy_model.predict(None, ["What was the revenue in q3?"])
...
# MAGIC %md
# MAGIC Our endpoint is now deployed! You can directly [open it from the UI](#/mlflow/endpoints/company_financial_transcript_rag) and visualize its performance!
# MAGIC
