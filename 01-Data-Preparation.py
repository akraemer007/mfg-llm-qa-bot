...
# MAGIC %md
# MAGIC
# MAGIC TODO create walkthrough in run notebook to show people where to put the documentsw
# MAGIC
...
# TODO Move into init
spark.sql(f"CREATE VOLUME IF NOT EXISTS {catalog}.{db}.{company_name}")
...
source_directory = f"/Volumes/{catalog}/custom_llm_demo/{company_name}/landing"
...
try:
    ...
    # TODO throw error if no documents were saved
    print("No new documents to load")
...
raw_documentation_tbl_nm = f'{company_name}_raw_documentation'
spark.createDataFrame(documents).write.mode('overwrite').saveAsTable(raw_documentation_tbl_nm)
spark.sql(f"ALTER TABLE {raw_documentation_tbl_nm} SET OWNER TO `account users`;")
display(spark.table(raw_documentation_tbl_nm).limit(20))
...
df = (
    spark.table(raw_documentation_tbl_nm)
...
spark.sql(f"CREATE TABLE IF NOT EXISTS {clean_documentation_tbl_nm}  (id BIGINT GENERATED BY DEFAULT AS IDENTITY, url STRING, content STRING, title STRING);")
spark.sql(f"ALTER TABLE {clean_documentation_tbl_nm} SET OWNER TO `account users`;")

if (
    not spark.catalog.tableExists(f"{catalog}.{db}.{clean_documentation_tbl_nm}")
    or spark.table(clean_documentation_tbl_nm).count() < 50
):
    df.write.mode("overwrite").saveAsTable(clean_documentation_tbl_nm)

spark.sql(f"ALTER TABLE {clean_documentation_tbl_nm} SET OWNER TO `account users`;")

# COMMAND ----------

display(spark.table(clean_documentation_tbl_nm))
